...
async function getAlbumList(world) {
  const albums = await world.page.$$eval(
    rowSel,
    (rows, userLinkSel, typeColumnSel) => {
      const albums = [];
      for (let row of rows) {
...
Given('a User Id {string} with albums', async function(v) {
  // Get full album list.
  await this.page.goto(global.APP_ADDRESS + routes.albums + '?perPage=1000');
  ...
  await this.page.goto(global.APP_ADDRESS + routes.albums + '?perPage=10');
