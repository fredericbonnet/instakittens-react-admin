...
async function getAlbumList(world, sel) {
  ...
  const albums = await world.eval(
    ...
    { dependencies: { sel, idColumnSel, typeColumnSel } }
  );
  return albums;
}
...
When('I get the Album list', async function() {
  await this.t.navigateTo(global.APP_ADDRESS + getAlbumListUrl(this));
});

Then('I should get the complete Album list', async function() {
  const location = await this.eval(() => window.location);
  await this.t.expect(location.hash).eql(getAlbumListUrl(this));
...
Then('the Album should be created as {string}', async function(v) {
  ...
  const location = await this.eval(() => window.location);
  await this.t.expect(location.hash).match(new RegExp(`${routes.albums}/\\d+`));
...
Then('the Album should not be created', async function() {
  await this.t.expect(this.albumCreationStatus).notOk();
});
...
Then('the Album {string} should equal the Album {string}', async function(
  ...
  await this.t.expect(album1).eql(album2);
});
Then('the Album {string} should include the Album {string}', async function(
  ...
  await this.t.expect(album1).contains(album2);
});
